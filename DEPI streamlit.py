...
def download_file_simple(url, filename):
    """Simple download function without complex error handling"""
    try:
...
@st.cache_resource
def load_models():
    """Load models with simple approach"""
...
if models is None:
    st.error("""
    **Failed to load models. This is usually because:**
    
    1. **Google Drive is blocking downloads** - Try the alternative solution below
    2. **Large file timeout** - The feature_selector.pkl is 689MB and may timeout
    
    **Quick Fix:** Let's try the alternative download method...
    """)
    
    if st.button("🔄 Try Alternative Download Method"):
        st.info("Using alternative download approach...")
        # We'll implement the fallback here
        pass
...
# Rest of your app code (the prediction interface) remains exactly the same...
...
st.markdown("---")
st.markdown("Deployed on Streamlit Community Cloud | Google Drive Models")
